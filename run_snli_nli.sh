...
# Script to run SNLI NLI classification with Ollama
...
PORT=8000
DATASET_PATH="datasets/snli_test.parquet"
OUTPUT_DIR="/work/11079/tiffanysh/vista/artifact-eval/output"
SAMPLE_SIZE=""  # Leave empty to use entire dataset, or set to a number

echo "=================================================="
echo "SNLI NLI Classification with Ollama"
echo "=================================================="
echo ""
...
mkdir -p logs/snli_nli
OLLAMA_HOST=0.0.0.0:$PORT OLLAMA_ORIGINS="*" $OLLAMA_PATH serve > logs/snli_nli/ollama_server.log 2>&1 &
SERVER_PID=$!
...
if [ $ELAPSED -ge $MAX_WAIT ]; then
    ...
    echo "Check logs at: logs/snli_nli/ollama_server.log"
    cat logs/snli_nli/ollama_server.log
...
# Step 5: Run NLI classification
echo "[5/5] Running NLI classification on SNLI dataset..."
echo "Dataset: $DATASET_PATH"
if [ -z "$SAMPLE_SIZE" ]; then
    echo "Using entire dataset"
else
...
# Build command
CMD="python -m src.pyspark.nli_classification -d \"$DATASET_PATH\" -m \"$MODEL_NAME\" -p $PORT --save-predictions \"$OUTPUT_DIR\" --predictions-format csv"
...
if [ $ANALYSIS_EXIT_CODE -eq 0 ]; then
    echo "✓ NLI classification completed successfully!"
else
    echo "✗ NLI classification failed with exit code $ANALYSIS_EXIT_CODE"
fi
echo "=================================================="
echo ""
...
if [ $ANALYSIS_EXIT_CODE -eq 0 ]; then
    ...
    echo "Server logs are available at: logs/snli_nli/ollama_server.log"
fi
