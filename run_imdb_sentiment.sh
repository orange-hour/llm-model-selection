...
# Simple script to run IMDB sentiment analysis with Ollama
...
echo "=================================================="
echo "IMDB Sentiment Analysis with Ollama"
echo "=================================================="
echo ""
...
mkdir -p logs/imdb_sentiment
OLLAMA_HOST=0.0.0.0:$PORT OLLAMA_ORIGINS="*" $OLLAMA_PATH serve > logs/imdb_sentiment/ollama_server.log 2>&1 &
SERVER_PID=$!
...
if [ $ELAPSED -ge $MAX_WAIT ]; then
    ...
    echo "Check logs at: logs/imdb_sentiment/ollama_server.log"
    cat logs/imdb_sentiment/ollama_server.log
...
echo "[5/5] Running sentiment analysis on sampled IMDB reviews..."
echo "Dataset: $DATASET_PATH"
echo "Sample size: 385 rows"
...
python -m src.pyspark.sentiment_analysis -d "$DATASET_PATH" -p $PORT --save-predictions "$OUTPUT_DIR" --predictions-format csv --model-name "$MODEL_NAME"
...
if [ $ANALYSIS_EXIT_CODE -eq 0 ]; then
    ...
    echo "Server logs are available at: logs/imdb_sentiment/ollama_server.log"
fi
